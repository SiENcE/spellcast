...
export class UIManager {
  ...
  setupEventListeners() {
    ...
    // Main functionality
    this.elements.tweetButton.addEventListener('click', this.createTweet);
    ...
    // Update tweet button event listener to handle media
    if (this.elements.tweetButton) {
      // Remove existing listener (if we can) and add a new one
      this.elements.tweetButton.removeEventListener('click', this.createTweet);
  ...
  setupMediaUploadUI() {
    ...
    // Insert media container before the tweet button
    const tweetButtonContainer = this.elements.tweetButton.parentNode;
    tweetForm.insertBefore(mediaContainer, tweetButtonContainer);
  ...
  createTweet() {
    ...
    try {
      ...
      if (content.length > this.tweetManager.MAX_TWEET_LENGTH) {
        alert(`Your message exceeds the maximum length of ${this.tweetManager.MAX_TWEET_LENGTH} characters. Current length: ${content.length}`);
        return;
      }

      // Create the tweet through the tweet manager
      this.tweetManager.createTweet(content);
